...
2. 状态和节点数据持久化到 RocksDB，不复用原项目 LevelDB 封装。
...
| LevelDB block view、wallet DB | 不迁移格式 | RocksDB + authenticated state + 独立 keystore |
...
- `domain_id = keccak256("ARBOR_DOMAIN_V1" || network_id || parent_domain_id || create_tx_hash)`。
...
RocksDB 是持久化引擎，不是状态根算法。Arbor 需要独立的 authenticated state commitment：v1 优先实现 Ethereum Merkle Patricia Trie（Keccak + RLP）承诺 domain account/storage，以获得最直接的 EVM proof 语义；M0 spike 必须验证 `alloy-trie` 或其他 Rust 实现是否适合增量更新、历史 root 和 RocksDB node store。若最终改用 sparse Merkle tree 作为账户树，必须通过 ADR 明确承认其 `eth_getProof` 和 Ethereum state root 不兼容。`domain_heads_root` 可使用独立、固定 key/value 编码的 sparse Merkle map，不改变 domain 内 Ethereum state root。
...
3. 收到 BFT commit 后，用一个开启同步 WAL 的 RocksDB `WriteBatch` 写入 immutable trie nodes、block/receipt/index 和 commit marker，最后更新 heads。
...
mempool 提交、P2P peer 管理和 RocksDB schema 不属于 `ConsensusEngine` 领域 trait。adapter 可以转换第三方类型，但第三方 block/vote 类型不得泄漏到 state、executor、RPC。
...
crates/
  ...
  arbor-storage/      # RocksDB schema、batch、migration、recovery
...
- RocksDB 损坏、崩溃时双签、snapshot 投毒和索引与状态根不一致。
...
| RocksDB 与 authenticated trie 被误当成同一层 | `StateCommitment`/node store 分层；根和 proof 黄金向量 |
