...
- parity-db 保存 block、authenticated trie、receipt、consensus safety state 和索引。
...
- ADR-003：state commitment；对 Ethereum MPT 候选做 parity-db 增量写、历史 root、proof、pruning PoC，并确定 `domain_heads_root` 的稀疏承诺。
- ADR-004：Malachite 与 `hotstuff_rs` 双候选安全审计；候选在 live 场景前即触发硬失败，按 fallback 完成同一 dummy application 的 4-validator 更新/掉线/持久化重启模型与最小安全规格。
...
- state candidate 必须从同一输入稳定生成 root/proof，并能从 parity-db 重启读取历史 root。
...
- parity-db columns、schema version、migration registry、atomic commit marker。
...
- parity-db reopen、schema mismatch、corrupt/missing node/log。
...
- proposal overlay、commit 后 parity-db transaction、abandoned proposal 回补 mempool。
...
- ADR-004 重新接受后的 BFT adapter、consensus direct network 和独立 durable safety store；不得假设 parity-db 的普通异步 overlay 等价于 pre-sign durable completion。
...
- benchmark/regression dashboard：execution、trie、parity-db、sync、consensus latency。
...
- Malachite 和 `hotstuff_rs` 当前均不得进入 production workspace；只有 ADR-004 按重新接受条件通过后，选中的实现才可进入。
- `revm`/alloy 升级必须跑 state tests、root vectors 和 RPC fixture；节点软件版本不能隐式改变 EVM revision。
- parity-db major upgrade 必须做旧库副本 migration/reopen/rollback 演练。
